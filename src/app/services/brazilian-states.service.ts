...
export class BrazilianStatesService {
  private readonly brazilianStateList: StatesListResponse = [
  ...
  getStates(): Observable<StatesListResponse> {
    return new Observable((observer) => {
      setTimeout(() => {
        observer.next(this.brazilianStateList);
        observer.complete();
      }, 1500)
    })
  }
  ...
  getStateDescription(stateId: number): string {
    const stateDescription = this.brazilianStateList.find(state => state.id === stateId)?.descricao
